...
then
	${HOME}/services/database/selfmanaged/mariadb/InitialiseMariaDB.sh
fi

if ( [ "`${HOME}/utilities/config/CheckConfigValue.sh DATABASEINSTALLATIONTYPE:MySQL`" = "1" ] )
then
	${HOME}/services/database/selfmanaged/mysql/InitialiseMySQLDB.sh
fi
...
then
	if ( [ "${CLOUDHOST}" = "digitalocean" ] )
	then
		${HOME}/services/database/dbaas/digitalocean/postgres/InitialisePostgresDB.sh
	fi
	if ( [ "${CLOUDHOST}" = "exoscale" ] )
	then
		${HOME}/services/database/dbaas/exoscale/postgres/InitialisePostgresDB.sh
	fi
	if ( [ "${CLOUDHOST}" = "linode" ] )
	then
		${HOME}/services/database/dbaas/linode/postgres/InitialisePostgresDB.sh
	fi
	if ( [ "${CLOUDHOST}" = "vultr" ] )
	then
		${HOME}/services/database/dbaas/vultr/postgres/InitialisePostgresDB.sh
	fi
fi
