...
/bin/chmod 777 ${HOME}/utilities/security/EnforcePermissions.sh
...
then
	...
	then    
		...
		if ( [ "${count}" = "5" ] )
		then
			${HOME}/services/email/SendEmail.sh "APPLICATION INSTALLATION FAILED" "I have not been able to successfully install your application database" "ERROR"
			exit
		fi	
	fi
...
${HOME}/services/email/SendEmail.sh "A DATABASE HAS BEEN SUCCESSFULLY BUILT" "A Database has been successfully built and primed as is rebooting ready for use" "INFO"
